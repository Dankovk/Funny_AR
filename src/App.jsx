...
import { Clone, Float as FloatImpl, Html, Mask, OrthographicCamera, useMask } from '@react-three/drei'
...
function Scene({ portal, ...props }) {
  ...
  useFrame((state) => {
    ...
    hand.current.rotation.x = MathUtils.lerp(hand.current.rotation.x, clicked ? -0.7 : -0.5, 0.2)
    hand.current.position.lerp({ x: v.x - 100, y: wheel.current + v.y, z: v.z }, 0.4)
    state.camera.zoom = MathUtils.lerp(state.camera.zoom, clicked ? 0.9 : 0.7, clicked ? 0.025 : 0.15)
